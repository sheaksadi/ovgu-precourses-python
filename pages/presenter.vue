...
/**
 * Empty the room's database, for a dry run before the room arrives.
 *
 * Testing the puzzles from a phone leaves a leaderboard full of the presenter,
 * and there is no other way to take a solve back. Two taps, because it cannot
 * be undone: the first asks, the second does it.
 */
const dbState = ref<'idle' | 'asking' | 'working' | 'done'>('idle')
let askTimer: any = null

const clearDatabase = async () => {
  if (dbState.value === 'idle') {
    dbState.value = 'asking'
    clearTimeout(askTimer)
    askTimer = setTimeout(() => { if (dbState.value === 'asking') dbState.value = 'idle' }, 4000)
    return
  }
  if (dbState.value !== 'asking') return
  dbState.value = 'working'
  try {
    await $fetch('/api/admin/database', {
      method: 'POST',
      headers: { 'x-deck-admin': ADMIN_PASSWORD },
      body: { action: 'clear' },
    })
    dbState.value = 'done'
    setTimeout(() => { dbState.value = 'idle' }, 2000)
  }
  catch {
    dbState.value = 'idle'
  }
    ...
    <div class="h-dvh overflow-hidden bg-gray-950 text-gray-200 font-sans flex flex-col">
      ...
      <header class="flex-none grid grid-cols-[1fr_auto_1fr] items-center gap-3 px-3 sm:px-4 py-1.5 border-b border-gray-800">
        ...
        <div class="flex items-center justify-end gap-2">
          ...
          <button
            type="button"
            class="px-2.5 py-1 border rounded-lg text-xs"
            :class="dbState === 'asking'
              ? 'bg-red-600 border-red-500 text-white font-bold'
              : 'bg-gray-900 hover:bg-gray-800 border-gray-800 text-gray-400'"
            :disabled="dbState === 'working'"
            title="Empty the database: every name, answer and solve"
            data-admin="clear-db"
            @click="clearDatabase"
          >
            {{ dbState === 'asking' ? 'Sure? Clears everything' : dbState === 'done' ? 'Database empty' : 'Clear database' }}
          </button>
